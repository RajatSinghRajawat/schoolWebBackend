...
exports.createOverview = async (req, res) => {
  try {
    ...
    const organiser = await mongoose.model('OrganisationRegister').findById(organizerId);
    if (!organiser) {
    ...
    const competition = new Competition({
      user_id: req.body.user_id,
      organizerId: organizerId, // Add organizerId
      overview: {
        name: req.body.name,
        image: imageUrl,
        description: req.body.description,
...
exports.getOverview = async (req, res) => {
  try {
    const competition = await Competition.findById(req.params.id).populate('organizerId'); // Populate organizerId
    if (!competition) {
      return res.status(404).json({ message: 'Competition not found' });
    }
    res.json(competition);
...
exports.updateOverview = async (req, res) => {
  try {
    ...
    const updatedCompetition = await Competition.findByIdAndUpdate(
      ...
      updateData,
      { new: true }
    ).populate('organizerId'); // Populate organizerId
...
exports.getAllCompetitions = async (req, res) => {
  try {
    const competitions = await Competition.find({}).populate('organizerId'); // Populate organizerId
    res.json(competitions);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.getCompetitionsByOrganizer = async (req, res) => {
  try {
    const { organizerId } = req.params;
    const competitions = await Competition.find({ organizerId }).populate('organizerId'); // Populate organizerId
    if (!competitions.length) {
      return res.status(404).json({ message: 'No competitions found for this organizer' });
    }
    res.json(competitions);
